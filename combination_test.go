...
func TestCombineDisjunctive(t *testing.T) {
	...
	cf := CombineDisjunctive(mf1, mf2)
	assert.InDelta(0.0, cf.Get(K()), tolerance)
	assert.InDelta(0.0525, cf.Get(K("red")), tolerance)
	assert.InDelta(0.075, cf.Get(K("yellow")), tolerance)
	assert.InDelta(0.03, cf.Get(K("green")), tolerance)
	assert.InDelta(0.1893, cf.Get(K("red", "yellow")), tolerance)
	assert.InDelta(0.1155, cf.Get(K("red", "green")), tolerance)
	assert.InDelta(0.1194, cf.Get(K("yellow", "green")), tolerance)
	assert.InDelta(0.4183, cf.Get(K("red", "yellow", "green")), tolerance)
	assert.True(cf.Valid())
}
